...
type config struct {
	...
	CacheAuth  bool         `conformance:"CACHE_AUTH" yaml:"cacheAuth"`             // whether to allow auth to be cached and reused between requests
...
func confFromEnv(env, tag string, vp reflect.Value) error {
	...
	if v.Kind() == reflect.Struct {
		...
		for i := 0; i < v.NumField(); i++ {
			...
			if tagVal != "" {
				...
				if err := confFromEnv(tagEnv, tag, v.Field(i).Addr()); err != nil {
					return fmt.Errorf("field failed %q: %w", v.Field(i).Type().Name(), err)
				}
			}
		}
		return nil
	...
	if mt, ok := vp.Interface().(interface{ UnmarshalText(b []byte) error }); ok {
		if err := mt.UnmarshalText([]byte(val)); err != nil {
			return fmt.Errorf("failed to unmarshal %q: %w", env, err)
		}
		return nil
	}
...
func confLegacyEnv(c *config) error {
	...
	if v := os.Getenv("OCI_ROOT_URL"); v != "" {
		fmt.Fprintf(os.Stderr, "WARNING: OCI_ROOT_URL is deprecated, use OCI_REGISTRY and OCI_TLS instead\n")
